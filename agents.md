...
### Web Dashboard (`ui/` + `ui_server/`)

The project includes a React + FastAPI web dashboard for visual code search,
...
| File | Purpose |
|------|---------|
| `ui_server/server.py` | Entry point — CLI args, uvicorn launch, browser auto-open |
| `ui_server/app.py` | FastAPI application factory — CORS, route registration, SPA fallback |
| `ui_server/dependencies.py` | Dependency injection — `get_server()` provides CodeSearchServer to routes |
| `ui_server/routes/search.py` | `POST /api/v1/search` — wraps `CodeSearchServer.search_code()` |
| `ui_server/routes/projects.py` | `GET/POST /api/v1/projects` — list and switch projects |
| `ui_server/routes/index.py` | `GET/POST/DELETE /api/v1/index/*` — status, run, clear |
| `ui_server/routes/settings.py` | `GET/PUT /api/v1/settings` — reads/writes `install_config.json` |
| `ui_server/routes/health.py` | `GET /api/v1/health` — liveness check |
| `ui_server/routes/models.py` | `GET /api/v1/models` — list available embedding models |
| `ui_server/static/` | Compiled React bundle (generated by `npm run build` in `ui/`) |

**React Frontend (`ui/src/`):**

| File | Purpose |
|------|---------|
| `ui/src/api/client.ts` | Typed HTTP client — all API calls and response types |
| `ui/src/store/useStore.ts` | Zustand store — query history, active project state |
| `ui/src/pages/SearchPanel.tsx` | Primary search interface — query input, filters, result list |
| `ui/src/pages/HealthDashboard.tsx` | Index statistics, sync status, charts, re-index controls |
| `ui/src/pages/SettingsForm.tsx` | Settings editor — model selection, reranker, idle memory |
| `ui/src/pages/ProjectsManager.tsx` | Project listing, switching, and indexing trigger |
| `ui/src/components/ResultCard.tsx` | Individual search result with code preview and copy actions |
| `ui/src/components/Layout.tsx` | Shell layout — sidebar navigation, content area |
| `ui/src/lib/utils.ts` | Pure utility helpers — clipboard, Markdown formatting, CSS merging |

**Dashboard design rules:**

- Routes are thin wrappers — all business logic stays in `CodeSearchServer`.
- `json.loads()` on server responses must always be wrapped in try/except.
- Settings routes access `install_config.json` through `common_utils` helpers only.
- The compiled frontend is served as static files; the SPA fallback returns `index.html` for all non-`/api/` paths.
- CORS allows only localhost origins (dev server at :5173 and production at :7432).
...
uv run --extra cu128 --extra test python -m pytest tests/unit/test_ui_server.py -v
